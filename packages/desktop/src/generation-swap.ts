...
export interface GenerationSwapOptions<Generation, Target, Supervisor extends GenerationSupervisor> {
  readonly projectId: string
  readonly port: number
  ...
  build(target: Target): Promise<Generation>
...
export async function swapProjectGeneration<Generation, Target, Supervisor extends GenerationSupervisor>(
  options: GenerationSwapOptions<Generation, Target, Supervisor>,
  ...
  let targetGeneration = options.describeTarget(options.target)
  const startedAt = now()
  ...
  try {
    const target = await options.build(options.target)
    targetGeneration = options.describeGeneration(target)
    ...
    candidate = await options.startSupervisor(target, { port: options.port, instanceId })
    await options.waitForReady(candidate, instanceId)
    await options.persistSelection(target)
